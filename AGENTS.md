...
- Phone gateway: `pnpm test:gateway` = real Caddy authentication/authority/Origin/WS boundaries + native inventory/events/terminal + phone Chromium/PWA without a private route + install rollback/password/flock/foreground ownership. Included in `pnpm verify`. Caddy pin/checksum = `scripts/caddy.mjs`; executable cache = ignored `.tools/`.
